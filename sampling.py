...
def save_stft_mag(wav, fname):
    ...
    plt.imshow(
        rosa.amplitude_to_db(
            stft(wav[0].detach().cpu()).numpy(), ref=np.max, top_db=80.0
        ),
        aspect="auto",
        origin="lower",
        interpolation="none",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--checkpoint", type=str, required=True, help="Checkpoint path"
    )
    ...
    parser.add_argument("--steps", type=int, required=False, help="Steps for sampling")
    parser.add_argument(
        "--no_init_noise",
        action="store_false",
        default=True,
    ...
    parser.add_argument(
        "--max_save", type=int, default=10, required=False, help="Maximum save samples"
    )
    ...
    wav = lp(wav, 0)
    swrite(
        f"{os.path.splitext(args.file_name)[0]}_{args.sr}.wav",
        hparams.audio.sr,
        wav[0].detach().cpu().numpy(),
    )

    save_stft_mag(wav, f"{os.path.splitext(args.file_name)[0]}_{args.sr}.png")
    upsampled = model.sample(wav, hparams.ddpm.max_step, args.no_init_noise, True)
    ...
    for i, uwav in enumerate(upsampled):
        ...
        if t > args.max_save:
            continue
        swrite(
            f"{os.path.splitext(args.file_name)[0]}_{t}.wav",
            hparams.audio.sr,
            uwav[0].detach().cpu().numpy(),
        )
