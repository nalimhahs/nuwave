...
class NuWave(pl.LightningModule):
    ...
    def set_noise_schedule(self, hparams, train=True):
        self.max_step = hparams.ddpm.max_step if train else hparams.ddpm.infer_step
        noise_schedule = (
            eval(hparams.ddpm.noise_schedule)
            if train
    ...
    @torch.no_grad()
    def sample(
        self, y_down, start_step=None, init_noise=True, store_intermediate_states=False
    ):
        ...
        step = torch.tensor(
            [start_step] * batch_size, dtype=torch.long, device=self.device
        )
        y_t = (
            torch.randn_like(y_down, device=self.device)
            if init_noise
    ...
    def validation_step(self, batch, batch_nb):
        wav, wav_l = batch
        step = torch.randint(0, self.max_step, (wav.shape[0],), device=self.device) + 1
        loss, y, y_low, y_noisy, eps, eps_recon = self.common_step(wav, wav_l, step)

        self.log("val_loss", loss, sync_dist=True)
        if batch_nb == 0:
            i = torch.randint(0, wav.shape[0], (1,)).item()
            y_recon = self.predict_start_from_noise(y_noisy, step - 1, eps_recon)
