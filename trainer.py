...
class EMACallback(Callback):
    ...
    def on_train_batch_end(
        ...
        for k, v in self.current_parameters.items():
            self.current_parameters[k].copy_(
                self.alpha * v + (1.0 - self.alpha) * self.last_parameters[k]
            )
...
def train(args):
    ...
    if args.restart:
        ckpt = torch.load(
            glob(
                os.path.join(
                    hparams.log.checkpoint_dir, f"*_epoch={args.resume_from}.ckpt"
                )
            )[-1],
            map_location="cpu",
        )
    ...
    if args.ema:
        ckpt = torch.load(
            glob(
                os.path.join(
                    hparams.log.checkpoint_dir, f"*_epoch={args.resume_from}_EMA"
                )
            )[-1],
            map_location="cpu",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Significant change occured, use this",
    )
    parser.add_argument(
        "-e",
        "--ema",
